...
def main():
	...
	# pqueue will contain (pid,priority,workposition) of a ready process
	pqueue = []	
	inputqueue = []
	outputqueue = []
	...
	while(True):
		...
		# if no process is running in cpu, we have to give cpu time to next process in queue according to their priority	
		if (processRunning == -1):			
			if (len(pqueue) != 0):
				# next process will be the one with least priority value i.e. most preferred
				processRunning,priority, ptime = pqueue[0]
		...
		# current process will run to completion before another process can execute
		if (processRunning != -1):			
			ptime -= 1
			# process burst completed			
			if (ptime == 0):				
				...
				processRunning = -1 # current process done
			else:
				nextop = None
		else:
			nextop = None

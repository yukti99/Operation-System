...
void* Searcher(void* arg){
		...
		sem_post(&searchMutex);
			search(); // multiple searches can occur simultaneously so no need to lock this CS
		sem_wait(&searchMutex);
			noSearchers--;
			if (noSearchers == 0){
...
void* Inserter(void* arg){
		...
		sem_post(&insertMutex);
			...
			sem_wait(&ins_Sem);
				insert(); // CS locked to provide mutual exclusion between two inserters
			sem_post(&ins_Sem);
